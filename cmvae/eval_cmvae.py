...
data_dir = 'F:/graduate student job/pycharm/AirSim_code/AirSim-Drone-Racing-VAE-Imitation-Windows-main/airsim_datasets/soccer_test'
read_table = True
latent_space_constraints = True
weights_path = 'F:/graduate student job/pycharm/AirSim_code/AirSim-Drone-Racing-VAE-Imitation-Windows-main/model_outputs/cmvae_con_train/cmvae_model_45.ckpt'
...
images_np1 = images_np[:50000, :]
if read_table is True:
    raw_table = raw_table[:50000, :]
...
# f = open('E:/FunctionMethod/test/z_snow_circular_itself_cosin_10k9.txt', 'w')
# for i in range(1000):
#     cos_simi = np.sum(z1[0] * z2[i]) / (np.linalg.norm(z1[0]) * np.linalg.norm(z2[i]))
#     f.write(str(cos_simi) + '\n')

# emd_distance = 0
# f = open('E:/FunctionMethod/test/z_desert_snow_wasserstein10.txt', 'w')
# for i in range(1000):
#     cdf_z1 = np.cumsum(z1[i]) / np.sum(z1[i])
#     cdf_z2 = np.cumsum(z2[i]) / np.sum(z2[i])
#     emd_distance = wasserstein_distance(cdf_z1, cdf_z2)
#     f.write(str(emd_distance)+'\n')
...
for i in range(1, num_imgs_display + 1):
    ...
    plt.axis('off')
    plt.imshow(img_display)
fig.savefig(os.path.join('F:/graduate student job/pycharm/AirSim_code/AirSim-Drone-Racing-VAE-Imitation-Windows-main/Pictures', 'reconstruction_results.png'))
plt.show()
...
plt.axis('off')
plt.imshow(img_display)
fig2.savefig(os.path.join('F:/graduate student job/pycharm/AirSim_code/AirSim-Drone-Racing-VAE-Imitation-Windows-main/Pictures', 'reconstruction_interpolation_results.png'))
plt.show()
...
for i in range(1, z_num_mural * n_z + 1):
    ...
    z = np.zeros((1, n_z)).astype(np.float)
    ...
    plt.axis('off')
    plt.imshow(img_display)
fig3.savefig(os.path.join('F:/graduate student job/pycharm/AirSim_code/AirSim-Drone-Racing-VAE-Imitation-Windows-main/Pictures', 'z_mural.png'))
plt.show()
